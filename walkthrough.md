...
- **Active Player Disconnection Turn-Skipping**:
  - **The Issue**: If a player leaves the game/disconnects while it is currently their turn, the lobby gets stuck on their turn indefinitely because they are offline and cannot make any moves.
  - **The Solution**: In `leaveRoom` inside gameManager.ts, we now detect if the disconnecting player is the active player. If they are, the server prints a history log and calls `skipDisconnectedTurns(game)`. This helper advances the turn to the next player, continuing to skip other disconnected players if necessary until it finds a connected player or the game naturally concludes.
