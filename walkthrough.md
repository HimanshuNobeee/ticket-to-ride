...
- **The Solutions**: 
  - Updated client components ([Board.tsx](file:///Users/coolhim/Desktop/react%20projects/ticket-to-ride/client/src/components/Board.tsx), [Deck.tsx](file:///Users/coolhim/Desktop/react%20projects/ticket-to-ride/client/src/components/Deck.tsx)) and server game action validators in [gameManager.ts](file:///Users/coolhim/Desktop/react%20projects/ticket-to-ride/server/src/gameManager.ts) to treat both `'PLAYING'` and `'LAST_ROUND'` as active, playable stages.
  - Implemented a countdown variable `lastRoundTurnsLeft` in `endTurn` in [gameManager.ts](file:///Users/coolhim/Desktop/react%20projects/ticket-to-ride/server/src/gameManager.ts). When `LAST_ROUND` is first triggered, `lastRoundTurnsLeft` is set to the number of players. It decrements by 1 at the end of each subsequent turn, ensuring every player (including the initiator) gets exactly one more turn before triggering the `GAME_OVER` stage.
